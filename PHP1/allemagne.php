...
$espagne=$db->query("SELECT * FROM ARTICLES WHERE pays='ALLEMAGNE' ORDER BY id ASC LIMIT $premier,$perpage");
?>
  <?php
...
<ul>
    ...
    for($i=1;$i<=$nbpage;++$i){
      ...
      if($i==$pagecourante){
       ?>
        <li class="lien"><a href="allemagne.php ?p=<?php echo $i ?>"><?php echo $i++ ?></a></li>
       <?php
      ...
      {
        ?>
        <li class="lien"><a href="allemagne.php ?p=<?php echo $i++ ?>">
        <?php echo $i ?></a></li>
